...
png('extract_maxvmem.png', height=4096, width=4096, bg="white", res=600)
txtalpha <- 0.25
redtxtalpha <- 0.5

par(mar=c(5, 5, 1, 1))
a <- read.csv('extract_maxvmem.txt', header=T)
head(a)
coefs <- coef(lm(ChainLength~MaxVMem, data = a))
# Sanity check
coefs
fitcoefs = coef(lm(ChainLength~0 + MaxVMem, data = a))
fitlmv_ChainLength <- as.numeric(fitcoefs[1])

# coefs contains two values: (Intercept) and MaxVMem
lmv_intercept <- as.numeric(coefs[1])
lmv_ChainLength <- as.numeric(coefs[2])

lm(a$ChainLength~a$MaxVMem)
fitcoefs

xlabel <- "maxvmem"
ylabel <- "Monomer chain length"
rvalue <- cor(a$ChainLength, a$MaxVMem)
...
p <- qplot(MaxVMem, ChainLength, main="ddg_monomer maxvmem vs chain length", data=a, xlab=xlabel, ylab=ylabel, shape = I(19), alpha = I(txtalpha)) + # label=ProThermID
