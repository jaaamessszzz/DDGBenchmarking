...
class BackrubDDGInterface(DDGMonomerInterface):
    ...
    def add_scores_from_cluster_rescore(self, output_dirs, prediction_structure_scores_table, prediction_id_field, score_method_id):
        ...
        prediction_id_output_directories = set()
        for output_dir in output_dirs:
            ...
            for wt_key, mut_key in zip(wt_keys, mut_keys):
                ...
                if os.path.isdir( os.path.join(output_dir, str(prediction_id)) ):
                    if len( os.listdir( os.path.join(output_dir, str(prediction_id)) ) ) == 800:
                        prediction_id_output_directories.add( (output_dir, prediction_id) )
        ...
        r.set_total_count( len(prediction_id_output_directories) )
        for output_dir, prediction_id in prediction_id_output_directories:
    ...
    def zip_prediction_id(self, prediction_id, root_dir, delete_if_exists = False):
